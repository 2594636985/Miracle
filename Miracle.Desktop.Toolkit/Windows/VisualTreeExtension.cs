...
namespace Miracle.Desktop.Toolkit.Windows
{
    public static class VisualTreeExtension
    {
        ...
        public static TDependencyObject GetParent<TDependencyObject>(this DependencyObject dependencyObject) where TDependencyObject : DependencyObject
        {
            DependencyObject parentObject = LogicalTreeHelper.GetParent(dependencyObject);

            if (parentObject == null) return default(TDependencyObject);

            TDependencyObject parent = parentObject as TDependencyObject;

            if (parentObject != null && parent != null)
            {
                return parent;
            }
            else if (parentObject != null)
            {
                return GetParent<TDependencyObject>(parentObject);
            }

            return default(TDependencyObject);
        }
    }
}
