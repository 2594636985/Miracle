...
namespace Miracle.AomiToDB.Mapping
{
	using Common;
	using Expressions;
	using Extensions;
	...
	public class MappingSchema
	{
		...
		public void SetGenericConvertProvider(Type type)
		{
			if (!type.IsGenericTypeDefinitionEx())
				throw new AomiToDBException("'{0}' must be a generic type.".Args(type));

			if (!typeof(IGenericInfoProvider).IsSameOrParentOf(type))
				throw new AomiToDBException("'{0}' must inherit from 'IGenericInfoProvider'.".Args(type));
